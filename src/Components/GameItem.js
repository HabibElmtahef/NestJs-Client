...
const GameItem = ({Game, DeleteGame, UpdateGame}) => {
    return (
        ...
        >
            <div className="card text-center bg-transparent" style={{boxShadow:"1px 1px 4px rgba(0,0,0,.25)", borderRadius:"20px"}} >
                <div className="card-header bg-dark text-white justify-content-center">
                    <Link to={`/detail/${Game.id}`} style={{color: "white"}} ><strong>{Game.title}</strong> </Link>
